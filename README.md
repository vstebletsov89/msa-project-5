...
Теперь вам предстоит спроектировать сбор и отображение основных метрик для мониторинга и оповещения, выбрав основные параметры мониторинга. Также вам предстоит выбрать и обосновать решение для логирования. Нужно продемонстрировать локально развёрнутое решение, которое в дальнейшем компания сможет применить в облачной инфраструктуре.

Для работы над заданием вам понадобится ранее разработанное Spring Batch-приложение.
...
- С помощью Docker Compose развернуть Prometheus, Grafana и ELK-стек.
- Реализовать отправку метрик Spring Batch-приложением через Prometheus Agent или JMX Exporter.
- Добавить логирование Spring Batch-операций. Настроить отправку логов в Elasticsearch через `filebeat`.
- Создать дашборды сбора метрик приложения и работающие оповещения через Prometheus и(или) Grafana, выбрав наиболее подходящее решение и обосновав его.
- Организовать централизованное логирование с ELK, продемонстрировав отображение собранных логов в Kibana.

Для выполнения задания предоставлены шаблоны Docker Compose, Prometheus, Grafana, ELK и `filebeat`-конфигураций. Можете воспользоваться ими, взяв из репозитория здесь.

🔍 В `README.md` лежит дополнительная информация по подготовке окружения.

## В результате у вас должно получиться:

- доработанная C4-диаграмма с добавленным решением по мониторингу/логированию/оповещению;
- документ с обоснованием выбранных для отправки метрик;
- доработанный Spring Batch Java-проект с добавленной отправкой логов и метрик;
- модифицированный Docker Compose файл с Prometheus, Grafana и ELK;
- созданы Grafana dashboard'ы для отображения метрик;
- сконфигурированы оповещения по 1–2 метрикам в Grafana/Prometheus;
- сконфигурирован сбор логов в ELK;
- документ, где вы обосновываете выбор способа отправки логов и конфигурации оповещений.
...
💡 В этом задании основной акцент сделайте на имплементацию трейсинга запросов в логировании. Клиентом может выступать простое `job`-приложение.

## Что нужно сделать

- Продемонстрировать отображение данных логов в ELK.

Загрузить их в директорию `Task6` вашего репозитория.

## Как сдать работу

В директории `Task1` должны лежать обоснование выбранного решения, файл/файлы с кодом/конфигурацией пайплайна и демонстрация его в локальном развёртывании в виде скринкаста или скриншотов.

В директорию `Task2` нужно загрузить файл с заполненной таблицей и выводами по обоснованию выбора решения, C4-диаграмма To Be, документ с наброском верхнеуровневого плана по имплементации и конфигурации решения.

В директории `Task3` должны быть конфигурационные файлы для создания Docker-образа, конфигурационные файлы `k8s job` для использования Docker-образа с `cron`-конфигурацией, видео или скриншоты с демонстрацией работы `k8s job` в MiniKube.

В `Task4` — документ с обоснованием решения, C4-диаграмма архитектуры системы с внедрённым Spring Batch-компонентом, `docker-compose` файл, Java-проект с доработанным Spring Batch-приложением, а также видео или скриншоты, чтобы продемонстрировать функционал разработанного решения.

В `Task5` ожидается проект решения мониторинга, логирования и оповещения. Это должна быть C4-диаграмма, где вы обоснуете выбранные для отправки метрики. Реализация решения — доработанный Spring Batch Java-проект, Docker Compose для Prometheus, Grafana и ELK, а ещё дашборд в Grafana. Файлы конфигурации для оповещений, сбора логов в ELK, а также с обоснованием выбора способа отправки логов.

В `Task6` — отображение логов ELK.